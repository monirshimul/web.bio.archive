...
const SelectInputBar = () => {
  ...
  return (
    <div>
      ...
      <p className="text-neutral-400 my-5 bg-gray-50 rounded-lg p-5 text-lg font-inter">
      ...
      <div className="flex relative">
        <div className="w-[100%] mx-auto flex items-center flex-wrap gap-2 p-5 border rounded-lg border-dashed">
          ...
          <div className="">
            <input
              ref={inputRef}
              className="h-[15px] p-5 focus:outline-none"
              type="text"
              value={searchTerm}
            ...
            />
            <ul className="max-h-[300px] overflow-y-scroll overflow-x-hidden absolute bg-gray-50 scrollbar-thin">
              {suggestions?.users?.map((user, index) =>
                !selectedUserSet.has(user.email) ? (
                  <li
                    key={index}
                    onClick={() => handleSelectUser(user)}
                    className=" flex gap-2 items-center my-2 border border-dashed rounded-lg p-2 w-full cursor-pointer mx-2 hover:bg-slate-200 transition-all"
                  >
